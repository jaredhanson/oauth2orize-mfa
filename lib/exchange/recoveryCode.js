...
module.exports = function(options, authenticate, issue) {
  ...
  return function recovery_code(req, res, next) {
    ...
    var client = req[userProperty]
      ...
      , code = req.body.recovery_code
      , scope = req.body.scope;
    
    if (!token) { return next(new TokenError('Missing required parameter: mfa_token', 'invalid_request')); }
    if (!code) { return next(new TokenError('Missing required parameter: recovery_code', 'invalid_request')); }
    ...
    function authenticated(err, user, info) {
      ...
      try {
        ...
        if (arity == 8) {
          issue(client, user, code, scope, req.body, info, req.authInfo, issued);
        } else if (arity == 7) {
          issue(client, user, code, scope, req.body, info, issued);
        } else if (arity == 6) {
          issue(client, user, code, scope, req.body, issued);
        } else if (arity == 5) {
          issue(client, user, code, scope, issued);
        } else { // arity == 4
          issue(client, user, code, issued);
        }
      } catch (ex) {
        return next(ex);
      }
