...
module.exports = function(options, issue) {
  ...
  if (!issue) { throw new TypeError('oauth2orize-mfa.otp exchange requires an issue callback'); }
  ...
  return function otp(req, res, next) {
    ...
    try {
      ...
      if (arity == 6) {
        issue(client, oneTimePassword, scope, req.body, req.authInfo, issued);
      } else if (arity == 5) {
        issue(client, oneTimePassword, scope, req.body, issued);
      } else if (arity == 4) {
        issue(client, oneTimePassword, scope, issued);
      } else { // arity == 3
        issue(client, oneTimePassword, issued);
      }
    } catch (ex) {
      return next(ex);
    }
